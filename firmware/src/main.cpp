/* ==========================================================================
   ESP32 FIRMWARE — FSR dance pad + I2S speaker
   --------------------------------------------------------------------------
   Responsibilities of this board:
     ...
     3. Play tones/music through the I2S amp + speaker
   ...
   Wiring (adjust pins to match your actual build):
     ...
     (each FSR wired as a voltage divider with a fixed resistor to GND)

     I2S Amp (e.g. MAX98357A):
     BCLK -> GPIO26
     LRC  -> GPIO25
     DOUT -> GPIO22
   ========================================================================== */
...
/* Non-blocking-ish tone player: writes a short burst of samples for a
   "hit sound" when a pad is pressed. Kept short so it doesn't stall
...
/* Distinct pitch per lane so hits sound different -- purely optional
   polish, easy to change. */
...
void setup() {
    Serial.begin(115200);
    delay(200);
    Serial.println("ESP32 DDR pad + speaker firmware starting...");
